...
// @version      3.0.3
// @description  针对雨课堂视频进行自动播放，配置AI自动答题（修正版）
// @author       风之子
// @license      GPL3
// @match        *://*.yuketang.cn/*
// @match        *://*.gdufemooc.cn/*
...
(() => {
  ...
  const Utils = {
    ...
    async getDDL() {
      ...
      const elementDurationMs = duration * 1000;
      const timeout = Math.max(elementDurationMs * 3, 10000);
      return timeout;
    }
  };
  ...
  const Store = {
    ...
    getAutoRun() {
      return localStorage.getItem(Config.storageKeys.autoRun) === '1';
    },

    setAutoRun(flag) {
      localStorage.setItem(Config.storageKeys.autoRun, flag ? '1' : '0');
    },

    clearAutoRun() {
      localStorage.removeItem(Config.storageKeys.autoRun);
    }
  };
  ...
  function createPanel() {
    ...
    doc.write(`
      <style>
        ...
        #btn-start { background-color: #1677ff; }
        #btn-start:hover { background-color: #f6ff00; color: black; }
        #btn-clear { background-color: #ff4d4f; }
        #btn-setting { background-color: #52c41a; }
        .settings-footer {
          text-align: center;
          margin-top: 12px;
          display: flex;
    ...
    ui.question.addEventListener('click', () => {
      window.parent.alert('作者：niuwh.cn（重构版 by Codex，修正版 by ChatGPT）');
    });
    ...
    const log = message => {
      ...
      if (ui.info.lastElementChild) {
        ui.info.lastElementChild.scrollIntoView({ behavior: 'smooth', block: 'end', inline: 'nearest' });
      }
    };
...
`;
  ...
  class V2Runner {
    ...
    async handleVideo(course) {
      ...
      await Utils.poll(() => isDeadline || Utils.isProgressDone(progressNode?.innerHTML), {
        interval: 5000,
    ...
    async handleCourseware(course) {
      ...
      const deadlinePassed = (tableData?.deadline || tableData?.end)
        ? (tableData.deadline < Date.now() || tableData.end < Date.now())
        : false;
  ...
  class ProOldRunner {
    ...
    run() {
      this.panel.log('准备进入下一节...');
      const leafDetail = document.querySelectorAll('.leaf-detail');
      let classCount = Store.getProClassCount() - 1;

      while (
        leafDetail[classCount] &&
        !leafDetail[classCount].firstChild?.querySelector('i')?.className?.includes('shipin')
      ) {
        classCount++;
      ...
      const current = leafDetail[classCount];
      if (!current) {
        this.panel.log('没有找到下一节课程');
        Store.clearAutoRun();
        return;
      }
      ...
      const link = current.querySelector('a[href]') || current.closest('a[href]');
      if (link && link.href) {
        this.panel.log('使用同页跳转进入课程');
        window.location.href = link.href;
        return;
      }

      current.querySelectorAll('a[target]').forEach(a => a.removeAttribute('target'));

      this.panel.log('未找到链接，尝试阻止新页并点击');
  ...
  function start() {
    ...
    if (matchURL.includes('yuketang.cn/v2/web') || matchURL.includes('gdufemooc.cn/v2/web')) {
      ...
      } else if (document.querySelector('.leaf-detail')) {
        new ProOldRunner(panel).run();
      } else {
        panel.resetStartButton('开始刷课');
        panel.log('当前 pro/lms 页面未识别到目录或下一节按钮');
      }
    } else {
      panel.resetStartButton('开始刷课');
