...
+ 2026.06.20 3.0.4  增强 web/v2 路线课件（PPT）翻页兼容性，支持更多幻灯片容器与翻页按钮/键盘翻页兜底，排除 swiper loop 重复 slide，识别已读/未读状态以跳过已读页；PPT 翻页间隔保持 3 秒；修复视频未自动点击播放、进度未满 100% 反复重播的问题；修复 kejian/PPT 播放后返回课程列表位置错误导致刷课中断的问题；AI 答题支持填空题；修复课程列表顶层作业/练习项被跳过的问题